...
function recalculation(e) {
    ...
    if (e && e.target.id == "Storage") storage_val = e.target.value
    if (e && e.target.id == "Transfer") transfer_val = e.target.value
    ...
    for (const key in data) {
        ...
        if (price_Storage.__proto__.constructor.name !== "Object") {
            ...
            for (const key in price_Storage) {
                document.querySelector(`#${key}`).onChange = recalculation
        ...
        item.style.width = current_price.toFixed(2) + "px"
    ...
    backblaze.style.width = current_vultr_price + "px"
    backblaze_price.innerText = current_vultr_price + "$"
